...
mds = nanmean(diag(S));  %mean of the entries on the main diagonal of S
ms = nanmean(nanmean(S));  %the mean of all entries of S
msr = nanmean(S,2);  %mean of all entries in row i of S
N = k^2*(mds-ms)^2;
D = (k-1)*(nansum(nansum(S.^2))-2*k*nansum(msr.^2)+k^2*ms^2);
