function Bubbles_Mapping(Parameters, Emulate, SaveAps)
%Bubbles_Mapping(Parameters, Emulate, [SaveAps = 0])
%
...
for Trial = 1 : length(Parameters.Conditions)
    ...
    CurrVolume = 1; PrevVolume = 0; SaveNow = 0;
    while CurrVolume <= Parameters.Volumes_per_Trial        
        % Generate bubbles
        if PrevVolume ~= CurrVolume && mod(CurrVolume-1, Parameters.Volumes_per_Stimulus) == 0
            % Regenerate bubbles
            [x y] = pol2cart(RandOri(1,Parameters.Number_of_Bubbles)/180*pi, rand(1,Parameters.Number_of_Bubbles) * Rect(4)/2 + Parameters.Fixation_Width(2));
            r = sqrt(x.^2 + y.^2)/4 + Rect(4)/200;
            x = x + Rect(3)/2;
            y = y + Rect(4)/2;
            % Matrix with bubble parameters
            Bs = [x-r; y-r; x+r; y+r];
            % Next volume
        ...
        % Create bubble aperture
        Screen('FillRect', CircAperture, [Parameters.Background 255]);    
        if CurrCondit
            Screen('FillOval', CircAperture, [0 0 0 0], Bs);
        end
        ...
        % Draw movie frame
        Screen('DrawTexture', Win, BgdTextures(CurrStim), StimRect, CenterRect([0 0 Rect(4) Rect(4)], Rect), BgdAngle+CurrCondit-90);
        % Draw aperture 
        ...
        % If saving apertures or movie
        if SaveNow == 1 && SaveAps == 1
            ...
            sv = Parameters.Volumes_per_Trial*(Trial-1)+CurrVolume;  % Frames in apertures to be saved in now
            ApFrm(:,:,sv:sv+Parameters.Volumes_per_Stimulus-1) = repmat(CurApImg, [1 1 Parameters.Volumes_per_Stimulus]);
            SaveNow = 0;
        elseif SaveAps == 2
            CurApImg = Screen('GetImage', Win, CenterRect([0 0 Rect(4) Rect(4)], Rect), 'backBuffer'); 
            CurApImg = rgb2gray(CurApImg);
        ...
        if sum(CurrEvents > 0 & CurrEvents < Parameters.Event_Duration)
            ...
            if isfield(Parameters, 'Event_Chars')
                % If string is used
                Screen('FillOval', Win, Parameters.Event_Colour(2,:), CenterRect([0 0 Parameters.Fixation_Width(1) Parameters.Fixation_Width(1)], Rect));
                DrawFormattedText(Win, Event_String(CurrEvents > 0 & CurrEvents < Parameters.Event_Duration), 'center', Rect(4)/2-8, Parameters.Event_Colour(1,:));
            else
