function outputimg = GaussianDot(inputimg, sigma, xpos, ypos, contr, segm)
%outputimg = GaussianDot(inputimg, sigma, xpos, ypos, [contr, segm])
%
% Draws a Gaussian dot with specified parameters into the input image.
%
% Parameters:
...
%           Gaussian                                                              
L = exp(-(X.^2)./(2*sigma.^2)-(Y.^2)./(2*sigma.^2)) * contr/2;
if size(segm,2) == 3
