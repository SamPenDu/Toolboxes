function VDM=Sam_FieldMap_preproc(fm_dir,epi_dir,pm_defs,sessname)
%
% Function to prepare fieldmap data for processing
% Adapted by DSS 13/7/11 for use with bias-corrected images
% 
...
if iscell(epi_dir)
   ...
   for sessnum=1:nsessions
      epi_all = strvcat(spm_select('List',epi_dir{sessnum},'^bf.*\.nii$'),spm_select('List',epi_dir{sessnum},'^bf.*\.img$'));
      epi_img{sessnum}=fullfile(epi_dir{sessnum},epi_all(1,:));
   end
elseif ischar(epi_dir)
   nsessions=1;
   sessnum=1;
   epi_all = strvcat(spm_select('List',epi_dir,'^bf.*\.nii$'),spm_select('List',epi_dir,'^bf.*\.img$'));
