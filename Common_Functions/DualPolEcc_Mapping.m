...
if ~isodd(Parameters.Session)
    Wedges = flipud(Wedges);
    Wedges = [Wedges(end); Wedges(1:end-1)];
    Rings = flipud(Rings);
end
% Add the blanks
Visible = [ones(length(Wedges),1); zeros(Parameters.Volumes_per_Cycle(3),1)];
Wedges = [Wedges; repmat(Wedges(1),Parameters.Volumes_per_Cycle(3),1)];
Rings = [Rings; repmat(Rings(1),Parameters.Volumes_per_Cycle(3),1)];
...
if strcmpi(Parameters.Ring_Scaling(1:3), 'log') 
    ...
    if ~isodd(Parameters.Session)
        Pixels_per_Vol = flipud(Pixels_per_Vol);
    end
...
while CurrVolume <= length(Wedges)
    ...
    % Draw movie frame
    Screen('DrawTexture', Win, BgdTextures(CurrStim), StimRect, CenterRect([0 0 Rect(4) Rect(4)], Rect), BgdAngle+CurrAngle);
    % Draw aperture
